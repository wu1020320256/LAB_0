...
%     tmp4 = 0;
%     tmp5 = 0;
%     lora_set.channel_choice = [tmp5, tmp4];
%     lora_set.channel_choice = [tmp5, tmp4, tmp3, tmp2];
%     lora_set.Preamble_channel = tmp5;
    ...
    figure(2);
    downchirp_cfo = [downchirp_4175_cfo, downchirp_3975_cfo, downchirp_3775_cfo, downchirp_3575_cfo, downchirp_3375_cfo, downchirp_3175_cfo];
    stft(downchirp_cfo, lora_set.sample_rate, 'Window',rectwin(256),'OverlapLength',128,'FFTLength',lora_set.fft_x);
    ...
    figure(3);
    for tmp_i = 1:6
        G1 = G0((12.25+tmp_i-1)*dine+1 : (12.25+tmp_i)*dine);
        dechirp_G1 = G1 .* downchirp_cfo(tmp_i,:);
        dechirp_1_fft = abs(fft(dechirp_G1));
    ...
    %     [d_downchirp, d_upchirp] = build_idealchirp_pre(lora_set);   % build ideachirp
    % 检测preamble
%     [Preamble_start_pos] = detect_preamble(G0, lora_set, d_downchirp);  
...
%     time_plot(G0, lora_set, d_downchirp_cfo, 50);
    % 分别解调preamble和payload，获取bin值
%     for plot_count = 1:9
%         subplot(3,3,plot_count);
%         stft(G0((258.25+plot_count)*dine+1:(259.25+plot_count)*dine), lora_set.sample_rate, 'Window',rectwin(128),'OverlapLength',0,'FFTLength',2048);
%         stft(G0((1+plot_count)*dine+1:(2+plot_count)*dine), lora_set.sample_rate, 'Window',rectwin(128),'OverlapLength',0,'FFTLength',2048);
%     end
...
%     disp(max_pos_array);
%     tmptmp = sum((max_pos_array-1)' == lora_set.pkg_vertification);
%     if tmptmp ~= 23
%         disp(i);
%     end
%     disp((max_pos_array-1)');
