...
% 读取配置和验证文件
[loraSet] = readLoraSet('sf10_BW125.json', 0.25e6);
...
%     [~, downchirp_bin] = max(dechirp_fft);
    ...
    chirp_tmp = signal(1:1*loraSet.dine);
%     dechirp = chirp_tmp .* d_downchirp_cfo;
%     dechirp_fft = abs(fft(dechirp, loraSet.dine));
%     dechirp_fft = dechirp_fft(1:fftX) + dechirp_fft(dine-fftX+1:dine);
%     plot(dechirp_fft);
%     stft(chirp_tmp, loraSet.sample_rate, 'Window',rectwin(64),'OverlapLength',32,'FFTLength',loraSet.fft_x);
%     chirp_tmp = chirp_tmp .* d_downchirp_cfo;
%     chirp_tmp = chirp_tmp(1:16:end);
    angle_tmp = zeros(1,length(chirp_tmp));
    for count = 1:length(chirp_tmp)-1
       iphase_1 = angle(chirp_tmp(count));
       iphase_2 = angle(chirp_tmp(count+1));
    ...
    angle_tmp(end) = angle_tmp(end-1);
%     angle_tmp = angle_tmp(1:8:end);
    plot(angle_tmp, '*');
    axis([-inf inf, -pi, pi]);
    tmp = 1;
