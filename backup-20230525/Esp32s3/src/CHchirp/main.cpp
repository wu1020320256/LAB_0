...
void setup() {
  ...
  while(preamble_channel == 0)
    preamble_channel = random(subchirp_num+1); // random in 0 to 4
  int state = radio.begin(channels[preamble_channel], bw*1000, sf, 5, 0x12, 10, 8);
...
void loop() {
  ...
  if (transmittedFlag == true) {
    ...
    preamble_channel = random(subchirp_num+1);
    while(preamble_channel == 0)
      preamble_channel = random(subchirp_num+1); // random in 0 to 4
    radio.setFrequency(channels[preamble_channel]);
  ...
  if (fhssChangeFlag == true) {
    ...
    if(hopsCompleted == 2){
      ...
      if(jump_flag)
        radio.setFrequency(channels[preamble_channel] + bw/bin + adjust_bw - bw);
      else
        radio.setFrequency(channels[preamble_channel] + bw/bin + adjust_bw);
    }
    ...
    if(hopsCompleted == 3){
      ...
      if(jump_part == 0 && jump_flag == true)
        ...
        // radio.setFrequency(channels[2] - bw*3/8 - bw + adjust_bw);
      else
        radio.setFrequency(channels[channel_jump_array[true_bin_count * 4 + 0]] - bw*3/8 + adjust_bw);
        // radio.setFrequency(channels[2] - bw*3/8 + adjust_bw);
    ...
    if(hopsCompleted >= 4){
      if(hopsCompleted % 4 == 1){
        if(jump_part < 2 || (jump_part == 2 && jump_flag == true))
          ...
          // radio.setFrequency(channels[4] + bw/8 - bw + adjust_bw);
        else
          radio.setFrequency(channels[channel_jump_array[true_bin_count * 4 + 2]] + bw/8 + adjust_bw);
          // radio.setFrequency(channels[4] + bw/8 + adjust_bw);
      }
        ...
        if(jump_part < 3 || (jump_part == 3 && jump_flag == true))
          ...
          // radio.setFrequency(channels[1] + bw*3/8 - bw + adjust_bw);
        else
          radio.setFrequency(channels[channel_jump_array[true_bin_count * 4 + 3]] + bw*3/8 + adjust_bw);
          // radio.setFrequency(channels[1] + bw*3/8 + adjust_bw);
        true_bin_count++;
      }
        ...
        if(jump_part < 1 || (jump_part == 1 && jump_flag == true))
          ...
          // radio.setFrequency(channels[3] - bw/8 - bw + adjust_bw);
        else
          radio.setFrequency(channels[channel_jump_array[true_bin_count * 4 + 1]] - bw/8 + adjust_bw);
          // radio.setFrequency(channels[3] - bw/8 + adjust_bw);
      }
    }
